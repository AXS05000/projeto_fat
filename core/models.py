...
class BaseInfoContratos(models.Model):
    ...
    data_inicio_cto = models.DateField('Data Inicio do Contrato', max_length=10)
    
    def __str__(self):
        return f'{self.cod_cliente} - {self.cargo} - {self.contrato} - {self.data_inicio_cto}'
...
class BaseCNPJ(models.Model):
    ...
    def __str__(self):
        return self.cnpj + ' - ' + self.nome_cliente
